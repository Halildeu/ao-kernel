...
|---|---|---|
| AO-GATE-5 | ⏳ Not started | Create two GitHub Apps (policy + release-gate); Vault seed real PEM at `gpp2/github/policy-private-key-pem` + `gpp2/github/release-gate-private-key-pem`; update `.env` to per-service form; `docker compose up -d --force-recreate`; verify per-service env in container |
| AO-GATE-6 | ⏳ Blocked on AO-GATE-5 | Real PR dry-run check-run evidence (release-gate posts check-run on PR; branch protection NOT yet required) |
| AO-GATE-7 | ⏳ Blocked on AO-GATE-5 | Deployment-protection callback review evidence (policy App posts callback on deployment) |
| AO-GATE-8 | ⏳ Blocked on AO-GATE-6 + AO-GATE-7 | Branch protection cutover: required ao-release-gate check; admin bypass YASAK |
