...
`manifest_cli_contract_mismatch` kapatıldı ve helper sözleşmesi hem fail
senaryosunu hem success senaryosunu ayırt edecek seviyeye getirildi.

Gözlenen auth ayrımı:
...
1. İlk preflight
   ...
   - bundled manifest smoke da aynı noktada bloklandı; parser/argv
     uyuşmazlığı kalmadı
2. Kontrollü re-login sonrası
   - `claude auth logout` + `claude auth login --claudeai`
   - helper `overall_status: pass`
   - doğrudan `claude -p "reply with exactly ok"` çağrısı `ok` döndürdü
3. Not
   - aynı gün `claude setup-token` ile üretilen uzun ömürlü token
     `CLAUDE_CODE_OAUTH_TOKEN` altında `Invalid bearer token` verdi; bu yol şu
     anda kanıtlanmış kurtarma aksiyonu değildir
...
1. operator prerequisite olarak session re-login + gerçek `claude -p` probe
   zorunluluğunu netleştirmek
2. disposable workspace üstünde gerçek success smoke evidence'ını saklamak
3. yalnız gerekirse API-key auth rotasını ayrıca doğrulamak
