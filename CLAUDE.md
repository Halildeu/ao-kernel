...
`ao-kernel` bir **JSON-first, policy-driven, fail-closed AI orchestrator** paketidir. `pip install ao-kernel` ile herhangi bir Python projesine kurulur ve o proje üzerinde AI agent orchestration, LLM routing, policy enforcement, evidence trail ve quality gate sağlar.

**Kaynak repo:** `Halildeu/autonomous-orchestrator` — tüm geliştirme orada yapılıyor. Bu repo o kaynak repo'daki `src/` kodunu dağıtılabilir Python paketi haline getirmektir.

## Mimari Kararlar (4 istişare sonucu — CNS-001..004)

Bu kararlar Claude + Codex (GPT-5.4) arasında 4 turda istişare edilerek alındı. Değiştirmek için yeni istişare gerekir.
...
**NEDEN facade+shim:** Kaynak repo'da 328 Python dosyası ve 691 import satırı `src.*` kullanıyor. Big-bang rename çok riskli. Facade yeni public API sağlar, shim eski importları korur. v2.0.0'da shim kaldırılır.

### 2. Dağıtım: PyPI (git URL değil)

```bash
pip install ao-kernel           # PyPI'den kur
