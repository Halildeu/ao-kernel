...
| `gh-cli-pr` walkthroughs and manifests | Beta (operator-managed preflight + live-write readiness probe) | Default helper yolu preflight (`python3 scripts/gh_cli_pr_smoke.py`), live-write probe ise explicit opt-in (`--mode live-write --allow-live-write --repo <owner>/<sandbox-repo> --head <branch> --base <branch>`) + disposable guard + create->verify->rollback zinciri ister. `GP-5.6a` ayrıca passing GP-5.5b local rollback raporunu precondition yapan `scripts/gp5_disposable_pr_write_rehearsal.py` wrapper'ını ekler ve sandbox branch delete verify kanıtı üretir. Bu yine support widening değildir; gerçek remote PR açılışı stable shipped support yüzeyi değildir |
