...
- **`cost_usd` reconcile (benchmark/internal contract)** — v3.7 PR-F2 closed the benchmark/internal gap. The benchmark-only `mock_transport._maybe_consume_budget` shim has been removed; the adapter-path `post_adapter_reconcile` middleware (v3.3.0 PR-C3) is the sole cost drainer. Fast-mode mock runs no longer drain the budget; `assert_cost_consumed` now only applies to historical artefacts. Use `assert_budget_unchanged` for the new fast-mode contract and `assert_spend_recorded_event(source="adapter_path")` for full-mode event-backed pins. This does **not** by itself widen the Public Beta support boundary; `PUBLIC-BETA.md` / `SUPPORT-BOUNDARY.md` still govern shipped vs beta vs deferred support claims.
- **Real-adapter full mode** — v3.7 PR-F1 landed the scaffold (option + marker + fixture + secret canonicalization + `context_pack_ref` pin). v3.7 PR-F2 added the first runnable smoke (`TestFullModeAdapterPathReconcile` via codex-stub), removed the benchmark-only `_maybe_consume_budget` cost shim, and wired the scorecard's event-backed `cost_source="real_adapter"` detection. External `claude-code-cli` / `gh-cli-pr` wiring + `governed_bugfix` full-mode smoke stay scope-out (F2.1+). See [`BENCHMARK-FULL-MODE.md`](BENCHMARK-FULL-MODE.md).
