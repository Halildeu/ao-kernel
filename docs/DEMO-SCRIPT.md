# FAZ-A Demo Script — End-to-End Governed Coding Flow
...
Place the demo workspace override at `.ao/policies/policy_worktree_profile.v1.json` (see [docs/WORKTREE-PROFILE.md §3](WORKTREE-PROFILE.md)):
...
Each step lists the CLI command, expected output summary, and the evidence events emitted (see [docs/EVIDENCE-TIMELINE.md §2](EVIDENCE-TIMELINE.md)).
...
Prints the chronological event list — one line per event with `ts`, `kind`, `actor`, and a one-line payload summary. See [docs/EVIDENCE-TIMELINE.md §8](EVIDENCE-TIMELINE.md).
...
The three adapters exercised above have full configuration and trouble-shooting walkthroughs in [docs/ADAPTERS.md](ADAPTERS.md):
