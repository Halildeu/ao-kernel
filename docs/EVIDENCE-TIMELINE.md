...
3. **What did the policies decide?** — every emitted `policy_checked` and `policy_denied` event is in the stream. In `v3.13.3`, that covers the shipped preflight scope; adapter CLI command enforcement is added in `v4.0.0b1`.
