...
| GP-5.6a disposable PR write rehearsal | Rehearsal / no support widening | `gp5-disposable-pr-write-rehearsal-report.schema.v1.json` remote side-effect rehearsal raporu vardır. `python3 scripts/gp5_disposable_pr_write_rehearsal.py --local-patch-report <gp55b.json> --allow-live-write --repo <owner>/<sandbox-repo> --base main --output json` yalnız passing GP-5.5b precondition + sandbox guard + create/verify/close/delete kanıtı üretir. `support_widening=false`, `production_remote_pr_support=false`, `arbitrary_repo_support=false` zorunludur |
