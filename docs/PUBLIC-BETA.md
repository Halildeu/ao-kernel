...
| `repo export-plan` root export preview | Beta / experimental preview-only | `python3 -m ao_kernel repo export-plan --project-root . --workspace-root .ao --targets codex,agents --output json` mevcut repo-intelligence artefakt digest'lerini okuyup yalnız `.ao/context/repo_export_plan.json` üretir ve aynı JSON planı stdout'a basar. Hedefler şimdilik `codex -> CODEX_CONTEXT.md` ve `agents -> AGENTS.md` preview kaydıdır. Root authority dosyası oluşturmaz, güncellemez, truncate etmez veya silmez; LLM çağrısı, network access, vector backend query/write, MCP tool, `context_compiler` auto-feed ve RI-5b write confirmation bu satırın dışında kalır |
| `repo export` confirmed create-only root export | Beta (operator-managed confirmed write) | `python3 -m ao_kernel repo export --project-root . --workspace-root .ao --targets codex,agents --confirm-root-export CONFIRM_RI5B_ROOT_EXPORT_V1 --output json` yalnız mevcut `.ao/context/repo_export_plan.json` intent artifact'ını tüketir. Exact confirmation token, explicit target allowlist ve enabled path-scoped coordination policy gerekir. İlk hedefler `codex -> CODEX_CONTEXT.md` ve `agents -> AGENTS.md`; yalnız absent root dosyaları create eder, matching existing root dosyasını unchanged no-op sayar, conflict/update/symlink/path escape/stale source/missing plan durumlarında fail-closed çalışır. Overwrite yok, LLM/network/vector backend/MCP/`context_compiler` yok, support widening yok; JSON result `repo-root-export-result.schema.v1.json` ile validate edilir ve `support_widening=false` taşır |
...
| GP-5 controlled patch/test lane | Rehearsal / no support widening | `gp5-controlled-patch-test-contract.schema.v1.json` design contract'ı ve `gp5-controlled-patch-test-rehearsal-report.schema.v1.json` local disposable worktree rehearsal raporu vardır. `python3 scripts/gp5_controlled_patch_test_rehearsal.py --approve-apply --output json` yalnız deterministic local patch preview/apply/test/rollback/cleanup kanıtı üretir. Bu satır runtime patch support, live remote PR, real-adapter live-write veya production support widening değildir. **GPP-7 preserves this as preparation/rehearsal evidence under `keep_rehearsal_only`; it remains non-production write-side evidence.** |
| GP-5.6a disposable PR write rehearsal | Rehearsal / no support widening | `gp5-disposable-pr-write-rehearsal-report.schema.v1.json` remote side-effect rehearsal raporu vardır. `python3 scripts/gp5_disposable_pr_write_rehearsal.py --local-patch-report <gp55b.json> --allow-live-write --repo <owner>/<sandbox-repo> --base main --output json` yalnız passing GP-5.5b precondition + sandbox guard + create/verify/close/delete kanıtı üretir. `support_widening=false`, `production_remote_pr_support=false`, `arbitrary_repo_support=false` zorunludur. **GPP-7 preserves this as preparation/rehearsal evidence under `keep_rehearsal_only`; it remains sandbox/disposable evidence, not production remote PR claim.** **GPP-8 also preserves this as preparation/rehearsal evidence under `keep_sandbox_only`; the remote PR live-write lane stays sandbox/disposable-only with sandbox-guard keyword requirement preserved.** |
| GP-5 full production rehearsal contract | Contract inventory / no support widening | `gp5-full-production-rehearsal-contract.schema.v1.json` GP-5.7 execution matrix'ini tanımlar: issue/task, repo intelligence, explicit context handoff, adapter reasoning, patch plan, controlled patch/test, disposable PR rehearsal ve rollback/closeout. En az üç clean rehearsal ve bir fail-closed rehearsal ister; fakat bu satır execution değildir. `support_widening=false` ve `production_platform_claim=false` zorunludur. **GPP-6c treats this contract as supporting preparation evidence, not protected live execution evidence.** **GPP-7 also preserves this contract as preparation evidence under `keep_rehearsal_only`; it is not production write-side candidate evidence.** **GPP-8 also preserves this contract as preparation evidence under `keep_sandbox_only`; it is not production remote PR candidate evidence.** |
| GP-5 full production rehearsal execution gate | Rehearsal aggregation / no support widening | `gp5-full-production-rehearsal-report.schema.v1.json` ve `python3 scripts/gp5_full_production_rehearsal.py --matrix-file <matrix.json> --output json` önceki GP-5.7a/GP-5.4a/GP-5.5b/GP-5.6a JSON raporlarını toplar. Pass için üç clean pass chain ve bir fail-closed chain gerekir. Komut canlı remote write çalıştırmaz, support widening yapmaz ve `production_platform_claim=false` zorunludur. **GPP-6c preserves this aggregation as rehearsal evidence; it is not GPP-6 protected live E2E evidence.** **GPP-7 also preserves this aggregation as preparation evidence under `keep_rehearsal_only`; it is not production write-side candidate evidence.** **GPP-8 also preserves this aggregation as preparation evidence under `keep_sandbox_only`; it is not production remote PR candidate evidence.** |
| GP-5 operations support package | Operations gate / no support widening | `gp5-operations-support-package.schema.v1.json` ve `python3 scripts/gp5_operations_support_package.py --output json` runbook, known-bugs, support-boundary ve branch-protection/required-checks karar yüzeylerini doğrular. Bu satır operability/readiness kanıtıdır; support widening veya production platform claim değildir |
| GP-5 production platform claim decision | Closeout decision / no support widening | `gp5-production-platform-claim-decision.schema.v1.json` ve `python3 scripts/gp5_platform_claim_decision.py --output json` GP-5 success criteria `BC-1..BC-10` üstünden production claim kararını üretir. Güncel karar `keep_narrow_stable_runtime`: stable runtime dar sınırda desteklidir, fakat general-purpose production platform claim verilmez. **GPP-3c (Faz 3) infazı sonrası BC-10 status `"exception"`** taşır: deliberate policy exception olarak kayıtlıdır (`real_adapter_usage_and_cost_evidence_missing` artık promotion_blockers listesinden çıkmıştır); live adapter execution gerekirse operator-bound supersession slice gerek. `support_widening=false` ve `production_platform_claim=false` zorunludur |
| GPP-3a real-adapter usage/cost evidence schema | Schema / simulated path ready / no support widening | `real-adapter-usage-cost-evidence.schema.v1.json` ve `python3 scripts/real_adapter_usage_evidence.py {emit-simulated,from-ledger-event,validate}` real-adapter usage/cost evidence için per-run schema + simulated emitter + spend-ledger event converter sağlar. Schema'nın root `oneOf` kontratı: `unavailable_reason=null` ⇒ tokens/cost non-null; `unavailable_reason in {usage_missing, token_unavailable, cost_unavailable}` ⇒ tokens/cost null. `evidence_class=simulated` ⇒ `live_adapter_execution=false`. **Bu satır GP-5.9 BC-10 closure'ı DEĞİLDİR**; BC-10 closure path kararı GPP-3b'de, infazı GPP-3c'de. `support_widening=false`, `production_platform_claim=false`, `live_adapter_execution=false` zorunludur |
| GPP-4a claude-code-cli failure-mode matrix schema | Schema / simulated coverage ready / no support widening | `claude-code-cli-failure-mode.schema.v1.json` ve `python3 scripts/claude_code_cli_failure_mode_evidence.py {emit-simulated,validate}` claude-code-cli adapter'ının 7 kanonik failure mode'u (auth_missing, binary_missing, timeout, prompt_denied, malformed_output, policy_denied, redaction) için aggregate matrix artifact + simulated coverage sağlar. Schema'nın `contains` invariantları + root `oneOf` bind: `evidence_class=simulated` ⇒ `live_adapter_execution=false`, `protected_run.observed=false`; `coverage` tam 7 unique canonical mode taşımalı. **Bu satır GPP-4 decision'u DEĞİLDİR**; promote_read_only / keep_operator_beta / defer kararı GPP-4b'de authoritative olarak keep_operator_beta seçildi (`.claude/plans/GPP-4b-KEEP-OPERATOR-BETA-DECISION.md`); infazı GPP-4c'de uygulandı (`.claude/plans/GPP-4c-KEEP-OPERATOR-BETA-INFAZ.md`). `support_widening=false`, `production_platform_claim=false`, `live_adapter_execution=false` zorunludur |
| GPP-4 claude-code-cli read-only adapter production decision | Closeout decision / no support widening | Decision: `gpp4_keep_operator_beta_authoritative_no_live_adapter_execution_no_support_widening_no_production_claim`. GPP-4a (Faz 1) failure-mode matrix schema + simulated coverage; GPP-4b (Faz 2) keep_operator_beta authoritative kararı; GPP-4c (Faz 3) `scripts/gp5_platform_claim_decision.py` BC-1 summary update + docs sync (`SUPPORT-BOUNDARY.md`, `KNOWN-BUGS.md`, `PUBLIC-BETA.md`) + M4 milestone done. `BC-1` status hâlâ `blocked` taşır; promotion blockers (`claude_code_cli_auth_operator_managed`, `kb001_claude_code_cli_operator_managed_auth`, `protected_live_adapter_gate_unattested`) korunur. claude-code-cli lane Beta (operator-managed) kalır; production-certified read-only promotion bir Option X supersession slice (operator-bound live execution, `live_adapter_execution_allowed=true` flip, three protected clean live runs + one protected fail-closed live run, `evidence_class=live` failure-mode matrix artifact) gerektirir. `support_widening=false`, `production_platform_claim=false`, `live_adapter_execution=false` zorunludur |
| GPP-6 read-only E2E execution decision | Closeout decision / no support widening | Decision: `gpp6_keep_rehearsal_only_authoritative_no_live_adapter_execution_no_support_widening_no_production_claim`. GPP-6a (Faz 1) preflight contract + schema (`gpp6-read-only-e2e-preflight.schema.v1.json`, `scripts/gpp6_read_only_e2e_preflight.py`) with `execution_status=blocked_by_upstream_gates`; GPP-6b (Faz 2) keep_rehearsal_only authoritative kararı (`.claude/plans/GPP-6b-READ-ONLY-E2E-DECISION.md`); GPP-6c (Faz 3) docs sync (`SUPPORT-BOUNDARY.md`, `KNOWN-BUGS.md`, `PUBLIC-BETA.md`) + M5 milestone done. GPP-6a preflight `execution_status` preserved as preparation evidence; closure path is "preparation/rehearsal evidence preserved" not live evidence. Future protected live read-only E2E rehearsal requires a separate operator-bound Option X supersession slice (`authorize_protected_live_e2e`): `live_adapter_execution_allowed=true` flip, protected workflow dispatch under `ao-kernel-live-adapter-gate` + `AO_CLAUDE_CODE_CLI_AUTH`, three protected clean runs + one protected fail-closed run with `evidence_class=live`. GPP-6 Option X slot is independent from GPP-4 Option X slot (production-certified read-only tier promotion). `support_widening=false`, `production_platform_claim=false`, `live_adapter_execution=false` zorunludur |
| GPP-7 controlled write-side production decision | Closeout decision / no support widening | Decision: `gpp7_keep_rehearsal_only_authoritative_no_write_side_production_candidate_no_live_adapter_execution_no_support_widening_no_production_claim`. M6 chain Faz 1 (first of three M6 slices: GPP-7 + GPP-8 + GPP-9). Existing GP-5.5a controlled patch/test contract + GP-5.5b controlled patch/test rehearsal + GP-5.6a disposable PR write rehearsal + GP-5.7a/5.7b full rehearsal stack preserved as preparation evidence; controlled patch/test lane stays rehearsal-only. M6 milestone closure reserved for GPP-9 with three evidence_refs (GPP-7 + GPP-8 + GPP-9). Future production-targeted controlled write evidence requires a separate operator-bound Option X supersession slice (`write_candidate_ready`) with an explicit production-candidate authorization flag and production-targeted write run evidence. `support_widening=false`, `production_platform_claim=false`, `live_adapter_execution=false` zorunludur |
| GPP-8 remote PR live-write promotion decision | Closeout decision / no support widening | Decision: `gpp8_keep_sandbox_only_authoritative_no_remote_pr_production_candidate_no_live_adapter_execution_no_support_widening_no_production_claim`. M6 chain Faz 2 (second of three M6 slices: GPP-7 + GPP-8 + GPP-9). Existing GP-5.6a disposable sandbox PR write rehearsal + GP-5.7a/5.7b full rehearsal stack preserved as preparation evidence; remote PR live-write lane stays sandbox/disposable-only. Sandbox-guard keyword requirement preserved on `scripts/gp5_disposable_pr_write_rehearsal.py`. M6 milestone closure reserved for GPP-9 with three evidence_refs (GPP-7 + GPP-8 + GPP-9). Future production-targeted remote PR live-write evidence requires a separate operator-bound Option X supersession slice (`remote_pr_candidate_ready`) with an explicit production-candidate authorization flag, production-targeted remote PR runs on non-sandbox repositories, operator-verified write semantics, and reclassification of the remote PR evidence. `support_widening=false`, `production_platform_claim=false`, `live_adapter_execution=false` zorunludur |
| GPP-9 final claim + M6 closeout (**autonomous program closure**) | Closeout decision / no support widening | Decision: `gpp9_keep_narrow_stable_runtime_authoritative_program_closed_no_live_adapter_execution_no_support_widening_no_production_claim`. M6 chain Faz 3 (third and final M6 slice: GPP-7 + GPP-8 + GPP-9). Existing GP-5.9 `keep_narrow_stable_runtime` decision preserved as formal program closure authority. M6 milestone closed (status `done`, three evidence_refs: GPP-7 + GPP-8 + GPP-9). **Autonomous GPP program reaches 7/7 milestones (100%) under preparation/rehearsal evidence preserved authority.** BC-1..BC-10 baseline + promotion_blockers list preserved without enum reclassification. Narrow stable runtime is the recorded supported surface. Future `promote_general_purpose_production` or `promote_general_purpose_beta` requires a separate **operator-bound supersession** PR with explicit production-platform-claim authorization, full or partial production matrix evidence, and operator-verified semantics — not a new autonomous GPP slice. `support_widening=false`, `production_platform_claim=false`, `live_adapter_execution=false` zorunludur |
| RI-7 repo-intelligence tier promotion readiness | Supersession readiness / no support widening | `repo-intelligence-tier-promotion-readiness.schema.v1.json` ve `python3 scripts/repo_intelligence_tier_promotion_readiness.py --output json` explicit repo-intelligence scan/index/query yüzeyinin future general-purpose production platform claim'e katkı verecek seviyeye aday olup olmadığını ölçer. Varsayılan sonuç `blocked_operator_bound_evidence_required` olmalıdır; explicit operator authorization, general-purpose platform claim authorization, guardrail hardening matrix, vector backend E2E evidence, wheel-installed scan/index/query smoke, operator-verified runtime semantics, cross-lane production matrix evidence, GP-5.9 reclassification plan ve support-boundary transition planı gelmeden tier yükseltmez veya production platform claim vermez. Bu satır `repo scan`, `repo index --dry-run`, `repo index --write-vectors` veya `repo query` satırlarını Beta/experimental sınırından çıkarmaz. `support_widening=false`, `production_platform_claim=false`, `live_adapter_execution=false` zorunludur |
