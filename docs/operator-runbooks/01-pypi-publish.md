# Runbook 01 — PyPI v4.1.0 Publish (P0-1)
...
- v4.1.0 tag pushed to `origin` (verify: `git tag --list v4.1.0`)
- Tag SHA matches main release commit (`git rev-parse v4.1.0` ↔
...
1. Confirm the tag is reachable:
   ...
   git rev-parse refs/tags/v4.1.0
   ```
...
3. Dispatch the workflow with the v-tag ref:
   ...
   gh workflow run publish.yml \
       ...
       --field ref=refs/tags/v4.1.0
   ```
4. Watch the run:
   ```bash
   gh run watch --repo Halildeu/ao-kernel
   ```
5. Confirm `https://pypi.org/project/ao-kernel/4.1.0/` resolves.
6. Confirm the GHCR container tag exists and matches.

## Verification

- PyPI page: `https://pypi.org/project/ao-kernel/4.1.0/`
- GHCR container: matching v4.1.0 digest
...
- `pip` users: `pip install ao-kernel==<previous>` to pin away from 4.1.0
- Maintainer: **yank** the broken release if necessary (this is reversible)
  and publish a corrective patch release (`v4.1.1`)
- Do NOT attempt to delete 4.1.0 from PyPI; deletion is not supported and
...
- `pypi.org` already has a `4.1.0` entry from an earlier accidental publish
