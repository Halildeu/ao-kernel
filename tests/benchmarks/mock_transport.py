...
def mock_adapter_transport(
    ...
    def _cli_dispatcher(
        *,
        ...
        # v3.7 F2 absorb (Codex iter-2 AGREE): the
        # `_maybe_consume_budget` shim was removed as the adapter-path
        # `post_adapter_reconcile` middleware (v3.3.0 PR-C3) is now
        # the sole cost drainer. Fast-mode mock runs do NOT drain the
        # budget any more; scorecard collector emits `cost_source=None`
        # for fast-mode. Full-mode smokes rely on real
        # `llm_spend_recorded` events with `source="adapter_path"`.
        try:
            result = _dispatch(
                manifest=manifest,
...
# v3.7 F2 absorb (Codex iter-2 AGREE): the `_maybe_consume_budget`
# shim was removed. The adapter-path `post_adapter_reconcile`
# middleware (v3.3.0 PR-C3) is now the sole cost drainer; benchmarks
# observe real spend via `llm_spend_recorded` events with
# `source="adapter_path"` in full mode, and fast-mode mock runs no
# longer drain the budget. Scorecard collector emits
# `cost_source=None` for the fast-mode no-drain path.
