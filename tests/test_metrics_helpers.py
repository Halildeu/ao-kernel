...
class TestLiveClaimsCount:
    def test_dormant_policy_returns_empty_dict(self, tmp_path: Path) -> None:
    ...
    def test_live_claims_counted_per_agent(self, tmp_path: Path) -> None:
        """With an enabled policy override, acquire two claims for
        two agents and confirm the snapshot reports the correct
        live counts."""
        policy_path = tmp_path / ".ao" / "policies" / "policy_coordination_claims.v1.json"
...
class TestListTerminalRuns:
    ...
    def test_malformed_state_file_skipped(self, tmp_path: Path) -> None:
        ...
        (run_dir / "state.v1.json").write_text("{ not valid json", encoding="utf-8")
        assert list_terminal_runs(tmp_path) == []
