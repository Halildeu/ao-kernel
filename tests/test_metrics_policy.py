...
class TestWorkspaceOverride:
    @staticmethod
    def _write_override(workspace_root: Path, doc: dict[str, Any]) -> None:
        path = workspace_root / ".ao" / "policies" / "policy_metrics.v1.json"
    ...
    def test_override_enables_advanced_labels(self, tmp_path: Path) -> None:
        self._write_override(
            tmp_path,
            _valid_policy_dict(
                enabled=True,
    ...
    def test_override_missing_falls_back_to_bundled(self, tmp_path: Path) -> None:
    ...
    def test_override_corrupt_json_raises(self, tmp_path: Path) -> None:
        ...
        path = tmp_path / ".ao" / "policies" / "policy_metrics.v1.json"
        path.parent.mkdir(parents=True, exist_ok=True)
...
class TestSchemaValidation:
    def test_override_invalid_allowlist_enum(self, tmp_path: Path) -> None:
    ...
    def test_override_missing_required_fields(self, tmp_path: Path) -> None:
    ...
    def test_override_extra_top_level_key_rejected(self, tmp_path: Path) -> None:
...
class TestLoadedPolicyIsHashable:
    def test_policy_is_hashable_for_caching(self, tmp_path: Path) -> None:
