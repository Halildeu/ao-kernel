...
def test_no_workflow_mutation() -> None:
    """E-1-4 scope is local-only hook; the E-1-4 slice's own write-set
    MUST NOT touch CI workflows (E-1-3 covers CI changelog enforcement).

    BLK-005 absorb: path-filtered diff (restrict to
    `.github/workflows/`) so unrelated workflow maintenance that
    ...
    assert not touched, (
        "E-1-4 (pre-commit hook) slice must not touch .github/workflows/ "
        f"(E-1-3 covers CI changelog enforcement). Touched: {touched}"
    )
